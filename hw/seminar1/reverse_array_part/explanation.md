...
В следующей строке эта ссылка может быть перепревязана к новому объекту (общее количество памяти не изменится). Далее следуют вызовы функций, новые объекты будут храниться в стеке вызовов, не занимая динамическую память. Таким образом эта функция имеет 
$$
O(1)
$$
сложность по памяти.