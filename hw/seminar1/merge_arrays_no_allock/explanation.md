...
- Мы просто размещаем элемент на единственное свобоное место и решаем задачу.
...
- Первым шагом алгоритма мы сравниваем наибольший элемент списка длины `M` (далее - `a`, подсписок объединённого) с наибольшим элементом списка длины `N`(далее - `b`):
    ...
    После перемещения элемента мы сдвигаем указатель в конце массива `M + N`. В любом случае ни один элемент не был потерян, и свободных мест хватает для переноса.
